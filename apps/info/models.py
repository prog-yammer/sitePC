...
class Players(models.Model):
    nickname = models.CharField("Никнейм", max_length=200, db_index=True)
    vk = models.CharField("Id ВК", max_length=200, default="-", db_index=True)
    vk_name = models.CharField("Имя ВК", max_length=200, default="-", db_index=True)
    reputation = models.IntegerField("Очки репутации", default=0,
                                     validators=[MinValueValidator(0), MaxValueValidator(100)])
    add_date = models.DateTimeField("Время прихода", auto_now_add=True, db_index=True)
