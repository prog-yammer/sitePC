    ...
    <div class="g-d-c g-d-10 g-centered">
        <form action="{{ app_path }}" method="post" novalidate>{% csrf_token %}
            <fieldset class="grp-module grp-module-login">
                <h1>
                    <span class="grp-admin-title">{% autoescape off %}{% if grappelli_admin_title %}{{ grappelli_admin_title }}{% else %}{% get_admin_title %}{% endif %}{% endautoescape %}</span>
                    <span class="grp-current-page">{% trans "Log in" %}</span>
                </h1>
